...
export default function CheckUpLanding() {
  ...
  const { form, setForm, agreed, setAgreed, submitting, succeeded, error, handleWhatsApp, handleEmail } =
    useLandingLeadForm("Medical Check-Up");
              ...
              {succeeded ? (
                ...
                <form style={{ padding: 32, display: "flex", flexDirection: "column", gap: 16 }} onSubmit={e => e.preventDefault()}>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button type="button" onClick={handleWhatsApp} disabled={!agreed || submitting} style={{ flex: 1, background: "#25D366", color: "#fff", padding: "14px 20px", borderRadius: 8, fontWeight: 700, fontSize: 15, border: "none", cursor: agreed ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: agreed ? 1 : 0.4 }}>
                      <MessageCircle size={16} /> {submitting ? "Sending..." : "Send via WhatsApp"}
                    </button>
                    <button type="button" onClick={handleEmail} disabled={submitting || !agreed} style={{ flex: 1, background: "#2884C0", color: "#fff", padding: "14px 20px", borderRadius: 8, fontWeight: 700, fontSize: 15, border: "none", cursor: agreed ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: agreed ? 1 : 0.4 }}>
